...
app.post('/post_file',function(req,res){  
  upload(req,res,function(err) {  
      ...
      uploadFile(req.file.path, req.file.filename ,res,req);
...
let filename=req.params.file_name.split('@');
console.log(filename);
retrieveFile(filename[0]+'/'+filename[1], res);
});

app.get('/home.svg' ,(request,response) =>{
  response.sendFile(path.join(__dirname + '/home.svg'));
});
...
function uploadFile(source,targetName,res,request){
  ...
  fs.readFile(source, function (err, data) {
      if(err) {
          ...
          const putParams = {
              Bucket      : 'hostit2',
              Key         :  request.session.username+"/"+targetName,
              Body        :  data
